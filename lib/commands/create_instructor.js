...
var CreateInstructor = /** @class */ (function () {
    function CreateInstructor() {
        ...
        this.relativeSrcCommandsDirectory = "/../../src/commands";
        this.questions = [
            {
                name: 'name',
                type: 'input',
    ...
    CreateInstructor.prototype.generateFiles = function (data) {
        ...
        // @ts-ignore
        var inputFile = path.normalize(__dirname + this.relativeSrcCommandsDirectory + path.sep + 'Templates' + path.sep + 'EventInstructor' + (this.extension)[data.language]);
        // @ts-ignore
        ...
        if (data.style !== 'nothing') {
            var inputStyleFile = path.normalize(__dirname + this.relativeSrcCommandsDirectory + path.sep + 'Templates' + path.sep + 'style' + '.' + data.style);
            // @ts-ignore
        ...
        fs.copyFile(inputFile, outputFile, function (err) {
            ...
            // @ts-ignore
            fs.writeFile(__dirname + _this.relativeSrcCommandsDirectory + path.sep + "user_default.json", JSON.stringify(data), function (err) {
                if (err)
                    throw err;
                userDefault = true;
            });
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        if (fs.existsSync(__dirname + _this.relativeSrcCommandsDirectory + path.sep + 'user_default.json')) {
                            // @ts-ignore
                            fs.readFile(__dirname + _this.relativeSrcCommandsDirectory + path.sep + 'user_default.json', 'utf8', function (err, data) {
                                if (err)
                                    reject(err);
